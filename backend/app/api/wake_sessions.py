...
@router.patch(
    ...
    response_model=WakeSessionResponse,
    status_code=status.HTTP_200_OK,
    summary="Resume Ringing on Snoozed Wake Session",
)
def resume_ringing_endpoint(
    session_id: int,
    db: Session = Depends(get_db),
):
    """Resume ringing on a currently snoozed wake session.

    Transition: 'snoozed' -> 'ringing'.
    """
    try:
        return wake_session_service.resume_ringing(db=db, session_id=session_id)
    except WakeSessionNotFoundError as exc:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=str(exc),
